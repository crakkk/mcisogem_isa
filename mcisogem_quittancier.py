...
class mcisogem_quittancier_log(osv.osv):
    _name = "mcisogem.quittancier.log"
    _description = 'Logs des quittances'

    _columns = {
        'benef_id': fields.many2one('mcisogem.benef', 'Bénéficiaire'),
        'quittancier_id': fields.many2one('mcisogem.quittancier', 'Quittance'),
        'type_avenant_id': fields.many2one('mcisogem.type.avenant', 'Type avenant'),
        'deb_periode': fields.date('Période du'),
        'fin_periode': fields.date('Date fin'),
...
class mcisogem_quittancier(osv.osv):
        ...
        if len(lesgroups) > 0:
            for group in lesgroups:
                ...
                if group_obj.name == "RESPONSABLE COMPTABLE" or group_obj.name == "UTILISATEUR COMPTABLE":
                    return self.pool.get('mcisogem.quittancier').search_count(cr, uid, [('etat_paiement', '!=', 'P')])
    ...
    _columns = {
        'detail_quittancier_ids': fields.one2many('mcisogem.detail.quittancier', 'mcisogem_quittancier_id',
                                                  "Récapitulatif des mouvements d'incorporation",
                                                  order="code_statut_benef"),

        'detail_quittancier_retrait_ids': fields.one2many('mcisogem.detail.quittancier.retrait',
                                                          'mcisogem_quittancier_id',
                                                          "Récapitulatif des mouvements de retrait",
                                                          order="code_statut_benef"),
        ...
        # Prochaine prime
        'montant_prochaine_prime' : fields.integer('Prochaine prime'),
        'deb_prochaine_prime': fields.date('Début'),
        'fin_prochaine_prime': fields.date('Fin'),
        'montant_prochaine_prime_annuelle' : fields.integer('Prochaine prime annuelle'),

        'affiche': fields.boolean(''),
        'etat_paiement': fields.selection([
            ('P', "Payé"),
            ('NP', "Non Payé"),
        ], 'Statut du paiement', required=True, readonly=True),
    ...
    # Validation du paiement de la quittance
    def payer(self, cr, uid, ids, context=None):

        return super(mcisogem_quittancier, self).write(cr, uid, ids, {'etat_paiement': 'P',
                                                                      'date_paiement': time.strftime("%Y-%m-%d",
                                                                                                     time.localtime())},
                                                       context=context)

    # Annulation de validation du paiement de la quittance
    def annuler_paiement(self, cr, uid, ids, context=None):
        return super(mcisogem_quittancier, self).write(cr, uid, ids, {'etat_paiement': 'NP', 'date_paiement': None},
                                                       context=context)
    ...
    def onchange_garant(self, cr, uid, ids, garant_id, context=None):
        if not garant_id:
            ...
            for col in lespolicesgarant:
                cr.execute("insert into mcisogem_quittancier_police_temp (create_uid,name) values(%s, %s)",
                           (uid, col['id'],))
    ...
    def onchange_type_avenan(self, cr, uid, ids, type_avenant_id, context=None):
        if not type_avenant_id:
            ...
            return {'value': {'objet_particulier': avenant_data.code_type_avenant,
                              'avenan_libelle': avenant_data.code_type_avenant}}
    ...
    def onchange_police(self, cr, uid, ids, police_id, type_avenant_id, deb=None, fin=None, context=None):
        vals = {}
        ...
        if deb and fin:
            deb = datetime.strptime(str(deb), '%Y-%m-%d')
        ...
        if police_id != False and type_avenant_id != False:
            ...
            avenant_search = self.pool.get('mcisogem.type.avenant').search(cr, uid, [('code_type_avenant', '=', 'AFA')])

            if avenant_search:
                avenant_fact = avenant_search[0]
            ...
            if code_avenant == 'ARS':
                ...
                # les beneficiiares à exclure du calcul : ceux qui ont déjà participé au calcul pour le même type d'avenant et pour la même période
                if deb and fin:
                    les_exclus = self.pool.get('mcisogem.quittancier.log').search(cr, uid, [
                        ('type_avenant_id', '=', type_avenant_id), ('deb_periode', '=', deb),
                        ('fin_periode', '=', fin), ('state', '=', 'done')])

                else:
                    les_exclus = self.pool.get('mcisogem.quittancier.log').search(cr, uid, [
                        ('type_avenant_id', '=', type_avenant_id), ('state', '=', 'done')])



                exclus = []
                for x in self.pool.get('mcisogem.quittancier.log').browse(cr,uid,les_exclus):
                    exclus.append(x.benef_id.id)

                les_exclus = exclus


                # On recupere tous les collèges de la police
                lescolleges = self.pool.get('mcisogem.college').search(cr, uid,
                                                                       [('police_id', '=', police_id)])

                for college_data in self.pool.get('mcisogem.college').browse(cr, uid, lescolleges):

                    # on recupere les statuts de beneficiares de la police

                    leshistopolice = self.pool.get('mcisogem.histo.police').search(cr, uid, [
                        ('code_college', '=', college_data.id)])
                    lesstatuts = self.pool.get('mcisogem.histo.police').browse(cr, uid, leshistopolice).statut_ids
                    ...
                    for statut in lesstatuts:
                        ...
                        lesprimes = self.pool.get('mcisogem.histo.prime').search(cr, uid, [
                            ('college_id', '=', college_data.id), ('statut_benef_id', '=', statut_data.id)],
                                                                                 order="id ASC")

                        for prime_data in self.pool.get('mcisogem.histo.prime').browse(cr, uid, lesprimes):

                            dt_effet_prime = prime_data.dt_eff_mod_prime
                            ...
                            lesbenefs = self.pool.get('mcisogem.benef').search(cr, uid, [('statut', '!=', 'R'), (
                                'college_id', '=', college_data.id), ('statut_benef', '=', statut_data.id), (
                                                                                             'dt_effet', '<=',
                                                                                             dt_fin_police), ('id', 'not in', les_exclus)])


                            for benef in self.pool.get('mcisogem.benef').browse(cr, uid, lesbenefs):

                                nbre_mois = relativedelta(dt_fin_exercice, dt_fin_police).months

                                surprime = 0
                                # si le beneficiaire a une surprime
                                if benef.avoir_surprime:

                                    # la surprime est defini au forfait
                                    if benef.type_surprime == 'F':
                                        surprime += benef.surprime

                                    # la surprime est defini par taux
                                    elif benef.type_surprime == 'T':
                                        surprime += (benef.surprime * prime_assure) / 100

                                    surprime = round((surprime * nbre_mois) / 12.0)

                                prime_assure += round((prime_data.prim_assure * nbre_mois) / 12.0)
                                prime_assure +=surprime
                                prime_sida += round((prime_data.prime_sida * nbre_mois) / 12.0)





                            lesbenefs_r = self.pool.get('mcisogem.benef').search(cr, uid, [('statut', '=', 'R'), (
                                'college_id', '=', college_data.id), ('statut_benef', '=', statut_data.id), (
                                                                                             'dt_effet', '<=',
                                                                                             dt_fin_police),(
                                                                                             'date_exclusion', '>',
                            ...
                            for benef in self.pool.get('mcisogem.benef').browse(cr,uid,lesbenefs_r):
                ...
                somme_prime = round(prime_assure) + round(prime_sida)
                ...
                return {'value': {'souscripteur_id': data.souscripteur_id['id'], 'courtier_id': data.courtier_id['id'],
                                  ...
                                  'fin_periode': data.dt_fin_exercice, 'affiche': True,
                                  'repartition_prime': data.repartition_prime,
                                  'periodicite_paiem': data.periodicite_paiem['id'],
                                  'imputation_acc_courtier': data.imputation_acc_courtier,
                                  'imputation_acc_cie': data.imputation_acc_cie, 'prime_tot_sans_sida': prime_assure,
                                  'prime_sida': prime_sida, 'mnt_quittance_emis': somme_prime, 'base': somme_prime,
                                  'show_exoneration': data.taxe_exon,
                                  'show_intermediaire': data.imputation_acc_courtier, 'show_taxe': data.tva_oui_non,
                                  'show_gestionnaire': data.imputation_acc_gestionnaire,
                                  'show_garant': data.imputation_acc_cie, 'show_taxe_enreg': data.taxe_enreg}}

            # Avenant de facturation
            if code_avenant == 'AFA':
                return {'value': {'souscripteur_id': data.souscripteur_id['id'], 'courtier_id': data.courtier_id['id'],
                                  'dt_emi_ave': data.dt_emi_ave, 'dt_fin_ave': data.dt_fin_exercice,
                                  'date_effet_police': data.dt_effet, 'deb_periode': data.dt_deb_exercice,
                                  ...
                                  'periodicite_paiem': data.periodicite_paiem['id'],
                                  'imputation_acc_courtier': data.imputation_acc_courtier,
                                  'imputation_acc_cie': data.imputation_acc_cie, 'show_exoneration': data.taxe_exon,
                                  'show_intermediaire': data.imputation_acc_courtier, 'show_taxe': data.tva_oui_non,
                                  'show_gestionnaire': data.imputation_acc_gestionnaire,
                                  'show_garant': data.imputation_acc_cie, 'show_taxe_enreg': data.taxe_enreg}}
            ...
            if code_avenant == 'AIN':
                ...
                # les beneficiares à exclure du calcul : ceux qui ont déjà participé au calcul pour le même type d'avenant et pour la même période
                if deb and fin:
                    les_exclus = self.pool.get('mcisogem.quittancier.log').search(cr, uid, [
                        ('type_avenant_id', '=', type_avenant_id), ('deb_periode', '=', deb),
                        ('fin_periode', '=', fin), ('state', '=', 'done')])
                ...
                for benef in self.pool.get('mcisogem.benef').browse(cr, uid, les_benef_search):
                    ...
                    l_histo_prime_search = self.pool.get('mcisogem.histo.prime').search(cr, uid,
                                                                                        [('police_id', '=', police_id),
                                                                                         ('statut_benef_id', '=',
                                                                                          statut_benef), (
                                                                                             'college_id', '=',
                    ...
                    for l_histo_prime_data in self.pool.get('mcisogem.histo.prime').browse(cr, uid,
                                                                                           l_histo_prime_search):
                ...
                exercice_search = self.pool.get('mcisogem.exercice.police').search(cr, uid, [('state', '=', 'actif'), (
                    'police_id', '=', police_id)])
                exercice_data = self.pool.get('mcisogem.exercice.police').browse(cr, uid, exercice_search)

                if self.pool.get('mcisogem.exercice.police').search_count(cr, uid, [('state', '=', 'actif'), (
                        'police_id', '=', police_id)]) == 0:
                ...
                if data.typ_prime == 1:
                    ...
                    if int(mode_repar_prime) == 1:
                        ...
                        surprime = (surprime * mois_a_acourir) / 12.0
                        somme_prime = prime_assure + prime_sida + surprime
                        ...
                        surprime = (surprime * mois_a_acourir) / 365
                        somme_prime = prime_assure + prime_sida + surprime

                if deb and fin:
                    return {
                        'value': {'souscripteur_id': data.souscripteur_id['id'], 'courtier_id': data.courtier_id['id'],
                                  'dt_emi_ave': data.dt_emi_ave, 'dt_fin_ave': data.dt_fin_exercice,
                                  'date_effet_police': data.dt_effet, 'affiche': True,
                                  'repartition_prime': data.repartition_prime,
                                  'periodicite_paiem': data.periodicite_paiem['id'],
                                  'imputation_acc_courtier': data.imputation_acc_courtier,
                                  'imputation_acc_cie': data.imputation_acc_cie,
                                  'prime_tot_sans_sida': prime_assure + surprime,
                                  'prime_sida': prime_sida, 'mnt_quittance_emis': somme_prime,
                                  'show_exoneration': data.taxe_exon,
                                  'show_intermediaire': data.imputation_acc_courtier, 'show_taxe': data.tva_oui_non,
                                  'show_gestionnaire': data.imputation_acc_gestionnaire,
                                  'show_garant': data.imputation_acc_cie, 'show_taxe_enreg': data.taxe_enreg}}
            ...
            if code_avenant == 'AMB':
                ...
                for college in lescolleges:
                    ...
                    college_search = self.pool.get('mcisogem.college').search(cr, uid,
                                                                              [('id', '=', college['college_id'])])
                    college_data = self.pool.get('mcisogem.college').browse(cr, uid, college_search)

                    cr.execute("select distinct statut_benef from mcisogem_benef where police_id =%s and college_id=%s",
                               (police_id, college['college_id']))
                    ...
                    for statut in lesstatuts:
                        ...
                        cr.execute(
                            "select  * from mcisogem_histo_prime where college_id=%s and police_id=%s and statut_benef_id=%s order by id DESC LIMIT 2",
                            (college['college_id'], police_id, statut['statut_benef'],))
                        ...
                        cr.execute(
                            "select  * from mcisogem_benef where statut != 'R' and police_id=%s and statut_benef=%s and college_id=%s and dt_effet <= %s",
                            (police_id, statut['statut_benef'], college['college_id'], dt_fin_prime))
                        lesbenefs = cr.dictfetchall()

                        cr.execute(
                            "select  * from mcisogem_benef where statut = 'R' and police_id=%s and statut_benef=%s and college_id=%s and dt_effet <= %s and date_exclusion between %s and %s",
                            (police_id, statut['statut_benef'], college['college_id'], dt_fin_prime, dt_effet_prime,
                             dt_fin_prime))
            ...
            if code_avenant == 'AMO':
                ...
                lescolleges = self.pool.get('mcisogem.college').search(cr,uid,[('police_id' , '=' , police_id)])
                ...
                for college_data in self.pool.get('mcisogem.college').browse(cr,uid,lescolleges):
                    ...
                    for statut in lesstatuts:
                        ...
                        print lesprimes
                        for prime_data in self.pool.get('mcisogem.histo.prime').browse(cr, uid, lesprimes):
                            dt_fin_prime = datetime.strptime(str(prime_data.dt_echea_pol), '%Y-%m-%d')
                            dt_effet_prime = datetime.strptime(str(prime_data.dt_eff_mod_prime), '%Y-%m-%d')
                            ...
                            lesbenefs = self.pool.get('mcisogem.benef').search(cr, uid, [('statut', '!=', 'R'), (
                                'college_id', '=', college_data.id), ('statut_benef', '=', statut_data.id), (
                                                                                             ...
                                                                                             dt_fin_prime), (
                                                                                             'creat_incorpo', '=',
                                                                                             'I'),('dt_effet' , '<=' ,dt_fin_prime ),('id', 'not in',
                                                                                                    les_exclus)])

                            for benef_data in self.pool.get('mcisogem.benef').browse(cr,uid,lesbenefs):

                                dt_effet_benef = datetime.strptime(str(benef_data.dt_effet), '%Y-%m-%d')

                                if dt_effet_benef < dt_effet_prime:
                                    dt_effet_benef = dt_effet_prime

                                # si le beneficiaire a une surprime
                                if benef_data.avoir_surprime:

                                    # la surprime est defini au forfait
                                    if benef_data.type_surprime == 'F':
                                        surprime = benef_data.surprime

                                    # la surprime est defini par taux
                                    elif benef_data.type_surprime == 'T':
                                        surprime = (benef_data.surprime * prime_assure) / 100

                                nbre_mois = relativedelta(dt_fin_prime, dt_effet_benef).months + 1
                                surprime = (surprime * nbre_mois) / 12.0
                                prime_assure += (prime_data.prim_assure * nbre_mois) / 12.0
                                prime_assure += surprime
                                prime_sida += (prime_data.prime_sida * nbre_mois) / 12.0

                            # on parcours les bénéficiaires retirés concernés par la prime en cours
                            lesbenefs = self.pool.get('mcisogem.benef').search(cr, uid, [('statut', '=', 'R'), (
                                'college_id', '=', college_data.id), ('statut_benef', '=', statut_data.id), (
                                                                                             'dt_effet', '<',
                                                                                             dt_fin_prime),('id','not in',
                                                                                                                  les_exclus)])

                            for benef_data in self.pool.get('mcisogem.benef').browse(cr, uid, lesbenefs):

                                dt_excl_benef = datetime.strptime(str(benef_data.date_exclusion), "%Y-%m-%d %H:%M:%S")
                                ...
                                if benef_data.avoir_surprime:
                                    ...
                                    # la surprime est defini au forfait
                                    if benef_data.type_surprime == 'F':
                                        surprime_r = benef_data.surprime

                                    # la surprime est defini par taux
                                    elif benef_data.type_surprime == 'T':
                                        surprime_r = (benef_data.surprime * prime_assure) / 100

                                surprime_r = (surprime_r * nbre_mois) / 12.0
                                prime_assure_r += (prime_data.prim_assure * nbre_mois) / 12.0
                                prime_assure_r+=surprime_r
                                prime_sida_r += (prime_data.prime_sida * nbre_mois) / 12.0
            ...
            # Avenant de Mouvement d'incorporations
            if code_avenant == 'AIC':
                prime_assure = 0
                prime_sida = 0
                surprime = 0
            ...
            # Avenant de Mouvement de retrait
            if code_avenant == 'ART':
                prime_assure = 0
                prime_sida = 0
                prime_assure_r = 0
                    ...
                    for statut in lesstatuts:
                        ...
                        for prime_data in self.pool.get('mcisogem.histo.prime').browse(cr, uid, lesprimes):
                            ...
                            for benef_data in self.pool.get('mcisogem.benef').browse(cr, uid, lesbenefs):
                                ...
                                dt_excl_benef = datetime.strptime(str(benef_data.date_exclusion),
                                                                  "%Y-%m-%d %H:%M:%S")
                                dt_excl_benef = dt_excl_benef.date()
            ...
            # Avenant de regularisation
            if code_avenant == 'ARE':
                nbre_avenant_fact = self.pool.get('mcisogem.quittancier').search_count(cr, uid,
                                                                                       [('police_id', '=', police_id), (
                                                                                           'type_avenant_id', '=',
                                                                                           avenant_fact)])
                ...
                for college in lescolleges:
                    ...
                    for statut in lesstatuts:
                        ...
                        cr.execute(
                            "select  * from mcisogem_histo_prime where college_id=%s and police_id=%s and statut_benef_id=%s order by id ASC",
                            (college['college_id'], police_id, statut['statut_benef'],))
                        lesprimes = cr.dictfetchall()
                        ...
                        for prime in lesprimes:
                            ...
                            cr.execute(
                                "select  * from mcisogem_benef where police_id=%s and statut_benef=%s and college_id=%s and dt_effet <= %s",
                                (police_id, statut['statut_benef'], college['college_id'], dt_fin_prime))
                            lesbenefs = cr.dictfetchall()
                            ...
                            for benef in lesbenefs:
                                ...
                                if benef['statut'] == 'R':
                                    dt_fin_benef = datetime.strptime(str(benef['date_exclusion']),
                                                                     '%Y-%m-%d %H:%M:%S').date()
                                    dt_fin_prime = dt_fin_benef
                ...
                les_avenants_fact = self.pool.get('mcisogem.quittancier').search(cr, uid,
                                                                                 [('police_id', '=', police_id), (
                                                                                     'type_avenant_id', '=',
                                                                                     avenant_fact)])
    ...
    def create(self, cr, uid, data, context=None):
        ...
        if nbre_benef == 0:
            raise osv.except_osv('Attention', "Il n'existe aucun bénéficiaire sur cette police")

        # nbre_ave = self.pool.get('mcisogem.avenant').search_count(cr, uid, [('police_id', '=', data['police_id']), (
        # 'type_avenant_id', '=', data['type_avenant_id']), ('state', '=', 'valid')])
        #
        # if nbre_ave == 0:
        #     raise osv.except_osv('Attention', "Vous devez d'abord créé un avenant de ce type sur la police")
        ...
        # on verifie si la police a un exercice actif
        exercice_search = self.pool.get('mcisogem.exercice.police').search(cr, uid, [('state', '=', 'actif'), (
            'police_id', '=', data['police_id'])])
        exercice_data = self.pool.get('mcisogem.exercice.police').browse(cr, uid, exercice_search)

        if self.pool.get('mcisogem.exercice.police').search(cr, uid, [('state', '=', 'actif'),
                                                                      ('police_id', '=', data['police_id'])]) == 0:
            raise osv.except_osv('Attention', "Cette police ne possède pas d'exercice actif")

        dt_deb_exercice = data['deb_periode']
        dt_fin_exercice = data['fin_periode']

        obj_pol_data = self.pool.get('mcisogem.police').browse(cr, uid, data['police_id'], context=context)

        # on verifie si la periode choisie correspond à  la periodicite de la police
        periodicite_data = self.pool.get('mcisogem.unite.temps').browse(cr, uid, obj_pol_data.periodicite_paiem.id)
        periodicite_nbre_jr = periodicite_data.nbre_jour


        dt_debut_periode = datetime.strptime(str(data['deb_periode']), "%Y-%m-%d")
        dt_fin_periode = datetime.strptime(str(data['fin_periode']), "%Y-%m-%d")

        # if (dt_fin_periode - dt_debut_periode).days > periodicite_nbre_jr:
        #     raise osv.except_osv(_('Attention !'), _('Le nombre de jours est supérieure à la périodicité de la prime.'))
        ...
        pol_data = self.pool.get('mcisogem.police').browse(cr, uid, data['police_id'])
        ...
        ave = self.pool.get('mcisogem.avenant').search(cr, uid, [('type_avenant_id', '=', data['type_avenant_id']),
                                                                 ('police_id', '=', data['police_id'])])
        ...
        # Prochaine prime
        repartition_prime = pol_data.periodicite_paiem.nbre_jour
        repartition_prime_mois = int(repartition_prime / 30)


        deb_prochaine_prime = datetime.strptime(str(data['deb_periode']), '%Y-%m-%d')  + relativedelta(months=repartition_prime_mois)
        deb_prochaine_prime = datetime.strptime(str(deb_prochaine_prime), '%Y-%m-%d %H:%M:%S').date()

        fin_prochaine_prime = datetime.strptime(str(deb_prochaine_prime), '%Y-%m-%d') + relativedelta(months=repartition_prime_mois) - timedelta(days=1)
        fin_prochaine_prime = datetime.strptime(str(fin_prochaine_prime), '%Y-%m-%d %H:%M:%S').date()


        data['deb_prochaine_prime'] = deb_prochaine_prime
        data['fin_prochaine_prime'] = fin_prochaine_prime
        ...
        if code_avenant == 'AFA':
            ...
            # on vérifie si un avenant initial a été émis pour cette police
            code_avenant_quittance = self.pool.get('mcisogem.quittancier').search_count(cr, uid, [
                ('police_id', '=', data['police_id']), ('avenan_libelle', '=', 'AIN'), ('state', '=', 'done')])

            if code_avenant_quittance == 0:
                raise osv.except_osv('Attention',
                                     "Aucune quittance de type <Avenant Initial> n'a été émise pour cette police")
            ...
            if code_avenant_quittance > 0:
                ...
                cr.execute(
                    'update mcisogem_quittancier set prime_tot_sans_sida=%s, prime_sida=%s , mnt_quittance_emis = %s , taxe_acc_nostro = %s , taxe_acc_courtier = %s , taxe_acc_assureur =%s ,cout_d_acte1=%s , cout_d_acte=%s , cout_d_acte_courtier = %s , cout_d_acte0 = %s, mnt_taxe_prime0 = %s , mnt_glob0=%s , total_plus_access=%s , base=%s where id = %s',
                    (prime_net_total, prime_net_SIDA, total_prime_net, mnt_accessoires_gest, mnt_accessoires_interme,
                     mnt_accessoires_assureur, total_accessoires, mnt_comxion_gest, mnt_comxion_courtier,
                     total_commission, taxe_enregistrement, montant_ttc, montant_hc_plus_access, total_prime_net,
                     quittance_id))
        ...
        if code_avenant == 'ARS':
            ...
            les_valeurs = \
                self.onchange_police(cr, uid, 0, data['police_id'], data['type_avenant_id'], data['deb_periode'],
                                     data['fin_periode'])['value']
            ...
            if code_avenant_quittance > 0:
                ...
                if type_prime == '1':
                    ...
                    # On vérifie le type d'accessoire compagnie - Forfait ou pourcentage
                    if data['type_accessoires_assureur'] == True:
                        mnt_accessoires_assureur = round(
                            (mnt_accessoires_gest * data['mnt_accessoires_assureur']) / 100)
                    else:
                        mnt_accessoires_assureur = data['mnt_accessoires_assureur']
                    ...
                    cr.execute(
                        'update mcisogem_quittancier set prime_tot_sans_sida=%s, prime_sida=%s , mnt_quittance_emis = %s , taxe_acc_nostro = %s , taxe_acc_courtier = %s , taxe_acc_assureur =%s ,cout_d_acte1=%s , cout_d_acte=%s , cout_d_acte_courtier = %s , cout_d_acte0 = %s, mnt_taxe_prime0 = %s , mnt_glob0=%s , total_plus_access=%s , base=%s where id = %s',
                        (
                            prime_net_total, prime_net_SIDA, total_prime_net, mnt_accessoires_gest,
                            mnt_accessoires_interme,
                            mnt_accessoires_assureur, total_accessoires, mnt_comxion_gest, mnt_comxion_courtier,
                            total_commission, taxe_enregistrement, montant_ttc, montant_hc_plus_access, total_prime_net,
                            quittance_id))
                    ...
                    # les_exclus = self.pool.get('mcisogem.quittancier.log').search(cr, uid, [
                    #     ('type_avenant_id', '=', data['type_avenant_id']), ('deb_periode', '=', data['deb_periode']),
                    #     ('fin_periode', '=', data['fin_periode']), ('state', '=', 'done')])
                    #
                    # exclus = []
                    # for x in self.pool.get('mcisogem.quittancier.log').browse(cr, uid, les_exclus):
                    #     exclus.append(x.benef_id.id)
                    ...
                    #
                    #
                    # lescolleges = self.pool.get('mcisogem.college').search(cr, uid, [('police_id', '=', data['police_id'])])
                    #
                    # for college_data in self.pool.get('mcisogem.college').browse(cr, uid, lescolleges):
                    #
                    #     # on recupere les statuts de beneficiares de la police
                    ...
                    #         # on parcours toutes les primes qui ont été définies pour le statut en cours
                    #         lesprimes = self.pool.get('mcisogem.histo.prime').search(cr, uid, [
                    #             ('college_id', '=', college_data.id), ('statut_benef_id', '=', statut_data.id)] , order='id ASC')
                    #
                    #         for prime_data in self.pool.get('mcisogem.histo.prime').browse(cr, uid, lesprimes):
                    #             dt_fin_prime = datetime.strptime(str(prime_data.dt_echea_pol), '%Y-%m-%d')
                    #             dt_effet_prime = datetime.strptime(str(prime_data.dt_eff_mod_prime), '%Y-%m-%d')
                    #
                    #             dt_fin_police = police_data.dt_resil_pol
                    #
                    #             if dt_fin_police:
                    #                 dt_fin_police = datetime.strptime(str(dt_fin_police), '%Y-%m-%d')
                    #
                    ...
                    #
                    #
                    #             lesbenefs_retire = self.pool.get('mcisogem.benef').search(cr, uid, [('statut', '=', 'R'), (
                    #                 'college_id', '=', college_data.id), ('statut_benef', '=', statut_data.id), (
                    #                                                                                'dt_effet', '<=',
                    #                                                                                dt_fin_police), (
                    #                                                                                'date_exclusion',
                    #                                                                                '>',
                    #                                                                                datetime.strftime(dt_fin_police,
                    #                                                                                                  "%Y-%m-%d %H:%M:%S")),
                    #                                                                            ('id', 'not in',
                    #                                                                             les_exclus)])
                    #
                    ...
                    #                         surprime += (benef_data.surprime * prime_assure) / 100
                    #
                    #                     surprime = round((surprime * nbre_mois_benef) / 12.0)
                    #
                    #
                    #
                    #                 prime_assure += round((prime_data.prim_assure * nbre_mois_benef) / 12.0)
                    #                 prime_assure += surprime
                    #                 prime_sida += round((prime_data.prime_sida * nbre_mois_benef) / 12.0)
                    #
                    #                 prime_statut_assure += round((prime_data.prim_assure * nbre_mois_benef) / 12.0)
                    #                 prime_statut_assure += surprime
                    #                 prime_statut_assure_sida += round((prime_data.prime_sida * nbre_mois_benef) / 12.0)
                    #
                    #
                    #             for benef_r in self.pool.get('mcisogem.benef').browse(cr, uid, lesbenefs_retire):
                    #                 dt_effet_benef = datetime.strptime(str(benef_r.dt_effet), '%Y-%m-%d')
                    #                 dt_retrait_benef = datetime.strptime(str(benef_r.date_exclusion),
                    #                                                      '%Y-%m-%d %H:%M:%S')
                    #
                    #
                    #                 dt_fin_exercice = datetime.strptime(str(quittance_data.fin_periode), '%Y-%m-%d')
                    #                 dt_deb_exercice = datetime.strptime(str(quittance_data.deb_periode), '%Y-%m-%d')
                    #                 nbre_mois = relativedelta(dt_fin_exercice, dt_deb_exercice).months + 1
                    #
                    ...
                    #                     surprime = round((surprime * nbre_mois_benef) / 12.0)
                    #
                    #                 prime_assure_r += round((prime_data.prim_assure * nbre_mois_benef) / 12.0)
                    #                 prime_assure_r += surprime
                    #                 prime_sida_r += round((prime_data.prime_sida * nbre_mois_benef) / 12.0)
                    #
                    #
                    #                 prime_statut_assure_r += round((prime_data.prim_assure * nbre_mois_benef) / 12.0)
                    #                 prime_statut_assure_r += surprime
                    #                 prime_statut_assure_sida_r += round((prime_data.prime_sida * nbre_mois_benef) / 12.0)

                            # if nbre_benef_incorpo > 0:
                            #     cr.execute(
                            #         "insert into mcisogem_detail_quittancier (mcisogem_quittancier_id ,college,code_statut_benef,st_creat_incorpo,effectif_det,nbre_jour_mois_det,nbre_jour_mois_exercice,type_jour_mois_det,prime_indivuel,prime_indivuel_sida,prime_indivuel_tot,prime_indivuel_sida_tot ) values(%s ,%s ,%s ,%s ,%s ,%s ,%s ,%s ,%s ,%s ,%s ,%s)",
                            #         (quittance_id, college_data.code_college, statut_data.lbc_fam_statut, 'C',
                            #          nbre_benef_incorpo, nbre_mois, nbre_mois, quittance_data.repartition_prime[0],
                            #          prime_statut_assure, prime_statut_assure_sida,
                            #          prime_statut_assure_sida + prime_statut_assure, prime_statut_assure_sida)
                            #     )
                            #
                            # if nbre_benef_retrait > 0:
                            #     cr.execute(
                            #         "insert into mcisogem_detail_quittancier_retrait (mcisogem_quittancier_id ,college,code_statut_benef,st_creat_incorpo,effectif_det,nbre_jour_mois_det,nbre_jour_mois_exercice,type_jour_mois_det,prime_indivuel,prime_indivuel_sida,prime_indivuel_tot,prime_indivuel_sida_tot) values(%s ,%s ,%s ,%s ,%s ,%s ,%s ,%s ,%s ,%s ,%s ,%s)",
                            #         (quittance_id, college_data.code_college, statut_data.lbc_fam_statut, 'C',
                            #          nbre_benef_retrait, nbre_mois, nbre_mois, quittance_data.repartition_prime[0],
                            #          prime_statut_assure_r, prime_statut_assure_sida_r,
                            #          prime_statut_assure_r + prime_statut_assure_sida_r, prime_statut_assure_sida_r)
                            #     )
        ...
        if code_avenant == 'AMO':
            ...
            if code_avenant_quittance > 0:
                ...
                if type_prime == '1':
                    ...
                    prochaine_prime = 0
                    montant_prochaine_prime_annuelle = 0
                        ...
                        for statut in lesstatuts:
                            ...
                            for prime_data in self.pool.get('mcisogem.histo.prime').browse(cr, uid, lesprimes):
                                ...
                                for benef in self.pool.get('mcisogem.benef').browse(cr, uid, lesbenefs):
                                    ...
                                    surprime = 0
                                    # On verifie si le benef a une surprime
                                    if benef.avoir_surprime:
                                        # la surprime est defini au forfait
                                        if benef.type_surprime == 'F':
                                            surprime = (benef.surprime / 12.0)

                                        # la surprime est defini par taux
                                        elif benef.type_surprime == 'T':
                                            surprime = ((benef.surprime / 12.0) * prime_assure) / 100

                                    prime_assure += round((prime_data.prim_assure * nbre_mois_benef) / 12.0) + surprime
                                    prime_sida += round((prime_data.prime_sida * nbre_mois_benef) / 12.0)


                                    prime_statut_assure += round((prime_data.prim_assure * nbre_mois_benef) / 12.0) + surprime
                                    prime_statut_assure_sida += round((prime_data.prime_sida * nbre_mois_benef) / 12.0)



                                    # insertion dans la table des logs

                                    log_data = {}
                                    log_data['benef_id'] = benef.id
                                    log_data['quittancier_id'] = quittance_data.id
                                    log_data['type_avenant_id'] = data['type_avenant_id']
                                    log_data['deb_periode'] = data['deb_periode']
                                    log_data['fin_periode'] = data['fin_periode']
                                    log_data['state'] = 'draft'

                                    self.pool.get('mcisogem.quittancier.log').create(cr, uid, log_data, context)
                                ...
                                for benef_r in self.pool.get('mcisogem.benef').browse(cr, uid, lesbenefs_retire):
                                    ...
                                    surprime_r = 0
                                    # On verifie si le benef a une surprime
                                    if benef_r.avoir_surprime:
                                        # la surprime est defini au forfait
                                        if benef_r.type_surprime == 'F':
                                            surprime_r = (benef_r.surprime / 12.0)

                                        # la surprime est defini par taux
                                        elif benef_r.type_surprime == 'T':
                                            surprime_r = ((benef_r.surprime / 12.0) * prime_assure) / 100



                                    prime_assure_r += round((prime_data.prim_assure * nbre_mois_benef) / 12.0) + surprime_r
                                    prime_sida_r += round((prime_data.prime_sida * nbre_mois_benef) / 12.0)

                                    prime_statut_assure_r += round((prime_data.prim_assure * nbre_mois_benef) / 12.0) + surprime_r
                                    prime_statut_assure_sida_r += round((prime_data.prime_sida * nbre_mois_benef) / 12.0)
                                ...
                                # Calcul pour la prochaine prime
                                lesbenefs = self.pool.get('mcisogem.benef').search(cr, uid,
                                                                                   [('statut', '!=', 'R'), (
                                                                                       'college_id', '=',
                                                                                       college_data.id), (
                                                                                        'statut_benef', '=',
                                ...
                                for benef in self.pool.get('mcisogem.benef').browse(cr, uid, lesbenefs):
                                    ...
                                    nbre_mois_benef_2 = relativedelta(fin_prochaine_prime,
                                                                      deb_prochaine_prime).months + 1

                                    prochaine_prime += round((prime_data.prim_assure * nbre_mois_benef_2) / 12.0)
                                    prochaine_prime += round((prime_data.prime_sida * nbre_mois_benef_2) / 12.0)

                                    montant_prochaine_prime_annuelle += prime_data.prim_assure
                                    ...
                                    if benef.avoir_surprime:
                                        ...
                                        # la surprime est defini par taux
                                            ...
                                            surprime += ((benef.surprime / 12.0) * prime_data.prime_assure) / 100

                                        prochaine_prime += surprime
                                        montant_prochaine_prime_annuelle+= benef.surprime
                    ...
                    cr.execute(
                        "update mcisogem_quittancier set montant_prochaine_prime = %s , montant_prochaine_prime_annuelle = %s  where id =%s ",
                        (prochaine_prime,montant_prochaine_prime_annuelle,quittance_id,)
                    )
                        ...
                        for statut in lesstatuts:
                            ...
                            for prime_data in self.pool.get('mcisogem.histo.prime').browse(cr, uid, lesprimes):
                                ...
                                for benef in self.pool.get('mcisogem.benef').browse(cr, uid, lesbenefs):
                                    ...
                                    prime_statut_assure += round(
                                        (prime_data.prim_assure * nbre_mois_benef) / 12.0) + surprime
                                    prime_statut_assure_sida += round((prime_data.prime_sida * nbre_mois_benef) / 12.0)
                        ...
                        for statut in lesstatuts:
                            ...
                            for prime_data in self.pool.get('mcisogem.histo.prime').browse(cr, uid, lesprimes):
                                ...
                                dt_effet_prime = datetime.strptime(str(prime_data.dt_eff_mod_prime), '%Y-%m-%d')
                                    ...
                                    # on parcours les bénéficiaires retirés concernés par la prime en cours
                                lesbenefs_retire = self.pool.get('mcisogem.benef').search(cr, uid,
                                                                                          [('statut', '=', 'R'), (
                                                                                              'college_id', '=',
                                                                                              college_data.id), (
                                                                                           'statut_benef', '=',
                                ...
                                for benef_r in self.pool.get('mcisogem.benef').browse(cr, uid, lesbenefs_retire):
                                    ...
                                    prime_assure_r += round(
                                        (prime_data.prim_assure * nbre_mois_benef) / 12.0) + surprime_r
                                    prime_sida_r += round((prime_data.prime_sida * nbre_mois_benef) / 12.0)

                                    prime_statut_assure_r += round(
                                        (prime_data.prim_assure * nbre_mois_benef) / 12.0) + surprime_r
                                    prime_statut_assure_sida_r += round(
                                        (prime_data.prime_sida * nbre_mois_benef) / 12.0)
        ...
        if code_avenant == 'AIN':
            ...
            code_avenant_quittance = self.pool.get('mcisogem.quittancier').search_count(cr, uid, [
                ('police_id', '=', data['police_id']), ('avenan_libelle', '=', 'AIN'),
                ('deb_periode', '=', data['deb_periode']),
                ('fin_periode', '=', data['fin_periode']), ('state', '=', 'done')])


            # On Vérifie si une quittance de type AI n'a pas déjà été émise
            if code_avenant_quittance == 0:
                ...
                # On Vérifie si police par statut bénéficiaire
                if type_prime == '1':
                    ...
                    for college_data in self.pool.get('mcisogem.college').browse(cr, uid, lescolleges):
                        ...
                        for statut in lesstatuts:
                            ...
                            nbre_benef_incorpo = self.pool.get('mcisogem.benef').search_count(cr, uid, [
                                ('statut_benef', '=', statut_data.id), ('statut', '!=', 'R'),
                                ('college_id', '=', college_data.id), ('police_id', '=', data['police_id']),
                                ('id', 'not in', les_exclus)])
                            ...
                            for prime_data in self.pool.get('mcisogem.histo.prime').browse(cr, uid, lesprimes):
                                ...
                                for benef in self.pool.get('mcisogem.benef').browse(cr, uid, lesbenefs):
                                    ...
                                    nbre_mois_benef = relativedelta(dt_fin_exercice, dt_deb_exercice).months + 1

                                    prime_A += round((prime_data.prim_assure * nbre_mois_benef) / 12.0)
                                    prime_A_SIDA += round((prime_data.prime_sida * nbre_mois_benef) / 12.0)
                                    ...
                                    if benef.avoir_surprime:
                                        ...
                                        # la surprime est defini au forfait
                                        if benef.type_surprime == 'F':
                                            surprime__A += (benef.surprime/12.0)

                                        # la surprime est defini par taux
                                        elif benef.type_surprime == 'T':
                                            surprime__A += ((benef.surprime/12.0) * prime_A) / 100
                                ...
                                for benef in self.pool.get('mcisogem.benef').browse(cr, uid, lesbenefs):
                                    ...
                                    nbre_mois_benef_2 = relativedelta(fin_prochaine_prime, deb_prochaine_prime).months + 1
                            ...
                            if nbre_benef_incorpo > 0 and lesbenefs:
                                cr.execute(
                                    ...
                                    (quittance_id, college_data.code_college, statut_data.lbc_fam_statut, 'C',
                                     nbre_benef_incorpo, nbre_mois, nbre_mois, quittance_data.repartition_prime[0],
                                     prime_A + surprime__A, prime_data.prime_sida, prime_A + prime_A_SIDA + surprime__A , prime_A_SIDA)
                                )
                ...
                raise osv.except_osv('Attention !',
                                     "Une quittance a déjà été calculée avec le type d'avenant AIN pour cette police et pour la même période")
                return False
        ...
        if code_avenant == 'AA':
            ...
            if code_avenant_quittance > 0:
                ...
                if type_prime == '1':
                    ...
                    cr.execute(
                        "select distinct college_id from mcisogem_benef where valide_quittance > %s and police_id=%s",
                        (0, data['police_id'],))
                    lescolleges = cr.dictfetchall()
                    prime_net_total = 0
                    ...
                    for college in lescolleges:
                        ...
                        cr.execute(
                            "select distinct statut_benef from mcisogem_benef where valide_quittance > %s and police_id =%s and college_id=%s",
                            (0, data['police_id'], college['college_id']))
                        lesstatuts = cr.dictfetchall()

                        # On parcours les statut de bénéficiaire
                        for statut in lesstatuts:
                            cr.execute(
                                "select  * from mcisogem_histo_prime where college_id=%s and police_id=%s and statut_benef_id=%s ORDER BY dt_eff_mod_prime DESC limit 1",
                                (college['college_id'], data['police_id'], statut['statut_benef'],))
                            lesprimes = cr.dictfetchall()[0]

                            statut_search = self.pool.get('mcisogem.stat.benef').search(cr, uid, [
                                ('id', '=', statut['statut_benef'])])
                            statut_data = self.pool.get('mcisogem.stat.benef').browse(cr, uid, statut_search)

                            if statut['statut_benef'] == 1:
                                cr.execute(
                                    "select  * from mcisogem_benef where valide_quittance > %s and police_id=%s and statut_benef=%s and college_id=%s and creat_incorpo = %s",
                                    (0, data['police_id'], 1, college['college_id'], 'C',))
                                lesbenefs = cr.dictfetchall()
                                ...
                                cr.execute(
                                    "insert into mcisogem_detail_quittancier (mcisogem_quittancier_id ,college,code_statut_benef,st_creat_incorpo,effectif_det,nbre_jour_mois_det,nbre_jour_mois_exercice,type_jour_mois_det,prime_indivuel,prime_indivuel_sida,prime_indivuel_tot,prime_indivuel_sida_tot ) values(%s ,%s ,%s ,%s ,%s ,%s ,%s ,%s ,%s ,%s ,%s ,%s)",
                                    (quittance_id, college_data.code_college, statut_data.lbc_fam_statut, 'C',
                                     len(lesbenefs), nbre_mois, nbre_mois, quittance_data.repartition_prime[0],
                                     prime['prim_assure'], prime['prime_sida'], prime_A + prime_A_SIDA, prime_A_SIDA)
                                )
                                ...
                                if statut['statut_benef'] == 3:
                                    cr.execute(
                                        "select distinct * from mcisogem_benef where valide_quittance > %s and police_id=%s and statut_benef=%s and college_id=%s and creat_incorpo = %s",
                                        (0, data['police_id'], 3, college['college_id'], 'C',))
                                    lesbenefs = cr.dictfetchall()
                                    ...
                                    cr.execute(
                                        ...
                                        (quittance_id, college_data.code_college, statut_data.lbc_fam_statut, 'C',
                                         len(lesbenefs), nbre_mois, nbre_mois, quittance_data.repartition_prime[0],
                                         prime['prim_assure'], prime['prime_sida'], prime_C + prime_C_SIDA,
                                         prime_C_SIDA)
                                    )
                                    ...
                                    cr.execute(
                                        "select * from mcisogem_benef where valide_quittance > %s and police_id=%s and statut_benef=%s and college_id=%s and creat_incorpo = %s",
                                        (0, data['police_id'], 2, college['college_id'], 'C',))
                                    lesbenefs = cr.dictfetchall()
                                    ...
                                    cr.execute(
                                        ...
                                        (quittance_id, college_data.code_college, statut_data.lbc_fam_statut, 'C',
                                         len(lesbenefs), nbre_mois, nbre_mois, quittance_data.repartition_prime[0],
                                         prime['prim_assure'], prime['prime_sida'], prime_E + prime_E_SIDA,
                                         prime_E_SIDA)
                                    )

                        les_benef_search = self.pool.get('mcisogem.benef').search(cr, uid, [
                            ('police_id', '=', data['police_id'])])
                        les_benef_police = self.pool.get('mcisogem.benef').browse(cr, uid, les_benef_search)

                        for benef in lesbenefs:
                            cr.execute("update mcisogem_histo_benef set valide_quittance=%s where id=%s",
                                       (2, benef['id']))
                ...
                # Si police par tranche d'age
                    ...
                    cr.execute(
                        "select distinct college_id from mcisogem_histo_benef where st_retr_excl!=%s and police_id=%s",
                        ('R', data['police_id'],))
                    lescolleges = cr.dictfetchall()
                    prime_net_total = 0
                    prime_net_SIDA_total = 0
                    ...
                    for college in lescolleges:
                        cr.execute(
                            "select distinct code_tranche_age from mcisogem_histo_prime where college_id=%s and police_id=%s",
                            (college['college_id'], data['police_id'],))
                        ...
                        for trancheage in lesstrancheage:
                            obj_tranche_age_data = self.pool.get('mcisogem.tranche.age').browse(cr, uid, trancheage,
                                                                                                context=context)
                            cr.execute(
                                "select distinct * from mcisogem_histo_prime where  police_id=%s and college_id=%s code_tranche_age=%s",
                                (data['police_id'], college, trancheage,))
                            lesprimes = cr.dictfetchall()
                            cr.execute(
                                "select distinct * from mcisogem_histo_benef where st_retr_excl!=%s and valide_quittance in %s and police_id=%s and cod_statut_benef=%s and college_id=%s st_creat_incorpo in %s id_tran_age=%s",
                                ('R', (1, 2), data['police_id'], 'E', college, ('C'), trancheage,))
                            lesbenefs = cr.dictfetchall()
							...
							VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"""
                                       , (quittance_id, college, quittance_data.date_effet_police, len(lesbenefs),
                                          mois_a_acourir, nbre_mois_exercice,
                                          quittance_data.repartition_prime, lesprimes['prim_assure'],
                                          lesprimes['Prime_sida'], prime_tranche, prime_tranche_SIDA,
                                          obj_tranche_age_data.debut_tranche, obj_tranche_age_data.fin_tranche))
                ...
                if mode_repar_prime == 1:
                    mois_a_acourir = datetime.strptime(obj_pol_data.dt_fin_exercice,
                                                       '%Y-%m-%d').month - datetime.strptime(obj_pol_data.dt_effet,
                                                                                             '%Y-%m-%d').month
                    ...
                    jours_a_acourir = datetime.strptime(obj_pol_data.dt_fin_exercice,
                                                        '%Y-%m-%d').day - datetime.strptime(obj_pol_data.dt_effet,
                                                                                            '%Y-%m-%d').day
                ...
                cr.execute(
                    'update mcisogem_quittancier set prime_tot_sans_sida=%s, prime_sida=%s , mnt_quittance_emis = %s , mnt_accessoires_gest = %s , mnt_accessoires_courtier = %s , mnt_accessoires_assureur =%s ,cout_d_acte1=%s , cout_d_acte=%s , cout_d_acte_courtier=%s , cout_d_acte0 = %s, mnt_taxe_prime0 = %s , mnt_glob0=%s , total_plus_access=%s , base=%s where id = %s',
                    (prime_net_total, prime_net_SIDA, total_prime_net, mnt_accessoires_gest, mnt_accessoires_interme,
                     mnt_accessoires_assureur, total_accessoires, mnt_comxion_gest, mnt_comxion_courtier,
                     total_commission, taxe_enregistrement, montant_ttc, montant_hc_plus_access, mnt_accessoires_gest,
                     quittance_id))
                ...
                raise osv.except_osv('Attention !',
                                     "Une quittance avec le type d'avemant AI doit être émise pour cette police!")
                return False
        ...
        if code_avenant == 'ARE':
            ...
            nbre_avenant_fact = self.pool.get('mcisogem.quittancier').search_count(cr, uid, [
                ('police_id', '=', data['police_id']), ('type_avenant_id', '=', avenant_fact)])
            ...
            if code_avenant_quittance > 0:
                ...
                if type_prime == '1':
                    ...
                    cr.execute("select distinct college_id from mcisogem_benef where police_id=%s",
                               (data['police_id'],))
                    lescolleges = cr.dictfetchall()
                    prime_net_total = 0
                    prime_net_SIDA_total = 0
                    ...
                    for college in lescolleges:
                        ...
                        cr.execute(
                            "select distinct statut_benef from mcisogem_benef where police_id =%s and college_id=%s",
                            (data['police_id'], college['college_id']))
                        lesstatuts = cr.dictfetchall()
                        ...
                        for statut in lesstatuts:
                            ...
                            cr.execute(
                                "select  * from mcisogem_histo_prime where college_id=%s and police_id=%s and statut_benef_id = %s ORDER BY id DESC LIMIT 1",
                                (college['college_id'], data['police_id'], statut['statut_benef']))
                            ...
                            cr.execute(
                                "select  * from mcisogem_benef where statut != 'R' and police_id=%s and statut_benef=%s and college_id=%s and dt_effet <= %s",
                                (data['police_id'], statut['statut_benef'], college['college_id'], dt_fin_prime))
                            ...
                            for benef_r in lesbenefs_retire:
                                ...
                                if benef_r['avoir_surprime']:
                                    ...
                                    # la surprime est defini au forfait
                                    if benef_r['type_surprime'] == 'F':
                                        surprime = benef_r['surprime']

                                    # la surprime est defini par taux
                                    elif benef_r['type_surprime'] == 'T':
                                        surprime = (benef_r['surprime'] * prime_assure) / 100
                            ...
                            for benef in self.pool.get('mcisogem.benef').browse(cr, uid, lesbenefs):
                                ...
                                prochaine_prime += round((prime['prim_assure'] * nbre_mois_benef_2) / 12.0)
                                prochaine_prime += round((prime['prime_sida'] * nbre_mois_benef_2) / 12.0)
                                ...
                                if benef.avoir_surprime:
                                    ...
                                    # la surprime est defini par taux
                                        ...
                                        surprime += ((benef.surprime / 12.0) * prime['prim_assure']) / 100
                    ...
                    cr.execute(
                        "update mcisogem_quittancier set montant_prochaine_prime = %s where id =%s ",
                        (prochaine_prime, quittance_id,)
                    )
    ...
    def button_to_valider(self, cr, uid, ids, context=None):
        ...
        les_logs =  self.pool.get('mcisogem.quittancier.log').search(cr,uid,[('quittancier_id' , '=' , ids[0])])

        for log in self.pool.get('mcisogem.quittancier.log').browse(cr,uid,les_logs):
            self.pool.get('mcisogem.quittancier.log').write(cr,uid,log.id,{'state': 'done'},context=context)

        quittance_data = self.browse(cr,uid,ids)

        code_avenant_quittance = self.pool.get('mcisogem.quittancier').search_count(cr, uid, [
            ('police_id', '=', quittance_data.police_id.id), ('deb_periode', '=', quittance_data.deb_periode),
            ('fin_periode', '=', quittance_data.fin_periode), ('state', '=', 'done')])

        if code_avenant_quittance:
            raise osv.except_osv('Attention !',
                                 "Une quittance a déjà été calculée avec le même type d'avenant sur la police pour la même période")
    ...
    def button_to_cancel(self, cr, uid, ids, context=None):
        ...
        for log in self.pool.get('mcisogem.quittancier.log').browse(cr, uid, les_logs):
            self.pool.get('mcisogem.quittancier.log').write(cr, uid, log.id, {'state': 'draft'}, context=context)
